...
double cutting_plane_algorithm(double *w, long m, int MAX_ITER, double C, double epsilon, SVECTOR **fycache, EXAMPLE *ex, 
  ...
  for (i=0;i<m;i++) {
    ...
    */
		...
		if(r) {
			//printf("Error in mosek_qp_optimize: %d\n",r);
			if(r==1295) {
				while(r==1295) {
...
int bb_score_comp(const void *a, const void *b) {
    BB_SCORE *aa = (BB_SCORE*)a;
    BB_SCORE *bb = (BB_SCORE*)b;

    if(aa->bb_score > bb->bb_score)
        return -1;
    if(bb->bb_score > aa->bb_score)
        return 1;
    if(aa->img_idx > bb->img_idx){
        return 1;
    }
    else if(aa->img_idx < bb->img_idx){
        return -1;
    }
    if(aa->bb_idx > bb->bb_idx){
        return 1;
    }
    else if(aa->bb_idx < bb->bb_idx){
        return -1;
    }
}
